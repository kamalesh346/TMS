...
export default function AdminPanel() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      // ❌ Removed date param since we're filtering locally
  ...
  // ✅ Local filtering for Date (works like TripAssignmentPage)
  const filteredBookings = bookings.filter((b) => {
    return (
      (filterStatus === "all" || b.status === filterStatus) &&
      (!filterVehicleType || b.vehicleTypeId === filterVehicleType) &&
      (!filterStartDate || b.requiredStartTime?.startsWith(filterStartDate))
    );
  });
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4 }}>
      ...
      <List>
        {paginatedBookings.map((booking) => (
          <ListItem
            ...
            onClick={() => handleOpenBookingDetails(booking.id)}
            sx={{ cursor: "pointer" }}
            secondaryAction={
      ...
      {/* ✅ Using Reusable Booking Details Modal */}
